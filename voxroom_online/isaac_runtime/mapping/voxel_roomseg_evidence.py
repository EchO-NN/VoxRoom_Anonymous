...
class VoxelRoomsegEvidenceConfig:
    ...
    min_free_z_cells_for_xy_free: int = 6
    unknown_ratio_min_for_xy_unknown: float = 0.70
    ...
    wall_min_occupied_z_cells_for_xy_wall: int = 1
    ...
    outside_unknown_ratio_min_for_xy_unknown: float = 0.70
    min_effective_range_z_cells_for_known_column: int = 3
    min_effective_range_ratio_for_occluded_wall: float = 0.70
    count_in_range_unknown_as_occupied_for_wall: bool = True
    navigation_unknown_overrides_vertical_map: bool = False
    ...
    nav_assisted_free_enabled: bool = False
    ...
    fill_small_unknown_holes_inside_vertical_free: bool = False
...
def classify_voxel_columns_for_roomseg(
    *,
    ...
    # Eq. (2): n_e counts the union of observed and sensor-range-supported
    # voxels; overlapping evidence must not be counted twice.
    effective_range_count = np.sum(
        free_bin | occupied_bin | sensor_in_range_bin, axis=0
    ).astype(np.uint16)
    ...
    occluded_wall_supported = (
        wall_generalized_raw
        & (unknown_ratio < float(cfg.unknown_ratio_min_for_xy_unknown))
        & (effective_range_count.astype(np.float32) / float(active_count)
           >= float(cfg.min_effective_range_ratio_for_occluded_wall))
    )
    # Strict and occluded walls are complementary criteria (Eq. 2).
    wall_ratio_raw = wall_actual_ratio_raw | occluded_wall_supported if bool(
        cfg.wall_use_generalized_occupied_ratio
    ) else wall_actual_ratio_raw
    ...
    wall = wall_ratio_raw & ~wall_free_block
    # Unknown/effective-range gates apply to occluded walls above. Strict walls
    # do not require the auxiliary sensor-range record.
    ...
    if np.any(nav_unknown_override):
        ...
        wall = wall_raw_before_nav_unknown & ~vertical_free & ~nav_unknown_override
