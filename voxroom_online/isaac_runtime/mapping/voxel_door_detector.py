...
class VoxelDoorDetectorConfig:
    ...
    upper_generalized_solid_ratio_min_active: float = 0.90
    upper_in_range_unknown_ratio_max: float = 0.30
    upper_free_ratio_max: float = 0.10
    ...
    min_seed_component_bbox_extent_cells: int = 1
...
def _door_scan_z_bounds(voxel_grid: VoxelOccupancyGrid3D, cfg: VoxelDoorDetectorConfig) -> tuple[float, float, str]:
    z_min = max(float(cfg.z_scan_min_m), float(voxel_grid.active_z_min_m))
    top = float(voxel_grid.active_z_max_m if voxel_grid.active_z_max_m is not None else voxel_grid.z_max_m)
    ...
    return float(z_min), float(top), "effective_structural_interval"
...
def classify_voxel_door_seeds(
    *,
    ...
    debug = {
        ...
        "voxel_door_turn_z_estimate_mode": "free_centroid_plus_half_cumulative_free_thickness",
...
def classify_voxel_door_seeds_centroid_ratio_vectorized(
    ...
    *,
    ...
    free_extent_cells = free_count.astype(np.float32)  # Eq. (4): cumulative thickness, not height span.
    ...
    # Use voxel-index units for the split itself (Eq. 4), avoiding float32
    # metre-coordinate rounding at a bin boundary.
    index_sum = np.sum(is_free * np.arange(z_count, dtype=np.float64)[:, None], axis=0)
    turn_index = index_sum / np.maximum(free_count, 1) + float(cfg.centroid_turn_extent_scale) * free_count
    turn_pos = np.where(finite_turn, np.ceil(turn_index - 1e-9), z_count).astype(np.int32)
    ...
    lower_actual_free = _gather_cum_before(np.cumsum(is_free, axis=0, dtype=np.int32), turn_pos)
    lower_unknown = _gather_cum_before(np.cumsum(is_unknown, axis=0, dtype=np.int32), turn_pos)
    lower_unknown_ratio = lower_unknown.astype(np.float64) / np.maximum(lower_total, 1)
    ...
    reject(lower_actual_free < int(cfg.min_lower_free_cells), "lower_free_cells_too_few")
    reject(lower_unknown_ratio > float(cfg.lower_unknown_ratio_max), "lower_unknown_ratio_too_high")
    ...
    reject(upper_unknown.astype(np.float64) > float(cfg.upper_in_range_unknown_ratio_max) * upper_total_active, "upper_in_range_unknown_ratio_too_high")
    ...
    lower_free_xy[:, :] = np.clip(lower_actual_free, 0, np.iinfo(np.uint16).max).reshape(shape).astype(np.uint16)
    ...
    debug_maps = {
        ...
        "voxel_door_lower_unknown_ratio_xy": lower_unknown_ratio.reshape(shape).astype(np.float32),
        "voxel_door_lower_actual_free_count_xy": lower_actual_free.reshape(shape).astype(np.int32),
...
_SEED_REJECT_CODES = {
    ...
    "lower_unknown_ratio_too_high": 26,
