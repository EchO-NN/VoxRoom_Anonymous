...
def _selection_profiles(args: argparse.Namespace) -> list[dict[str, float | str]]:
    ...
    if not bool(args.allow_relaxed_selection):
        return [requested]
    return [
        requested,
        {
    ...
    for profile in _selection_profiles(args):
        try:
            episode, candidates = select_longest_instance_astar_episode(
                ...
                static_reference_radius_m=float(args.static_reference_radius_m),
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--static-reference-radius-m", type=float, default=0.05)
    parser.add_argument("--allow-relaxed-selection", action="store_true", default=False)
    ...
    for scene_dir in scenes:
        ...
        try:
            ...
            episode.setdefault("metadata", {})["robot_radius_m"] = float(
                args.robot_radius_m
            )
            episode.setdefault("metadata", {})["episode_set"] = (
                "strict_static_execution_contract"
            )
    ...
    report = {
        ...
        "robot_radius_m": float(args.robot_radius_m),
        "static_reference_radius_m": float(args.static_reference_radius_m),
        "runtime_planning_clearance_m": float(args.min_planning_clearance_m),
        "relaxed_selection_enabled": bool(args.allow_relaxed_selection),
        "rows": report_rows,
    }
