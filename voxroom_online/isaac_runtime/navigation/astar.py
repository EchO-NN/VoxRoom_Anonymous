...
def planner_distance_map(planner: GridAStarPlanner, start: GridCell) -> np.ndarray:
    start_free = planner.snap_to_free(start)
    dist = np.full(planner.traversible.shape, np.inf, dtype=np.float64)
    if start_free is None:
        return dist
    clearance_penalty = getattr(planner, "clearance_penalty", None)
    ...
    while heap:
        ...
        for dr, dc, step_cost in planner.neighbors:
            ...
            if not planner.can_step(cell, dr, dc):
                continue
            if callable(clearance_penalty):
                penalty = float(clearance_penalty(nbr))
                if not math.isfinite(penalty):
                    continue
                step_cost = float(step_cost) * (1.0 + penalty)
...
def astar_distance_map(traversible: np.ndarray, start: GridCell, resolution_m: float, allow_diagonal: bool = True) -> np.ndarray:
    ...
    return planner_distance_map(planner, start)
