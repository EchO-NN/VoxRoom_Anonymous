...
class CommittedFrontierExecutionState:
    ...
    def update_progress(
        ...
        resolution_m: float,
        progress_min_delta_m: float,
        progress_distance_m: float | None = None,
    ) -> float:
        distance_m = (
            float(progress_distance_m)
            if progress_distance_m is not None
            else self.distance_to_target_m(current_grid, resolution_m)
        )
