...
def category_from_semantic_label(label: str) -> str:
    parts = [part for part in str(label).replace("\\", "/").split("/") if part]
    if not parts:
        return normalize_category(label)
    for part in parts:
        category = normalize_category(part)
        if category not in GENERIC_SEMANTIC_LABELS:
            return category
    return normalize_category(parts[0])


def choose_semantic_label(labels: Sequence[str], prim) -> str:
    for label in labels:
        category = category_from_semantic_label(label)
        if category not in GENERIC_SEMANTIC_LABELS:
            return str(label)
    return fallback_label_from_prim_name(prim)
...
def extract_objects_from_usd(
    ...
    for prim in stage.Traverse():
        ...
        if not should_consider_prim(prim):
            continue
        labels = collect_semantic_labels(prim)
        raw_label = choose_semantic_label(labels, prim) if labels else fallback_label_from_prim_name(prim)
        category = category_from_semantic_label(raw_label)
