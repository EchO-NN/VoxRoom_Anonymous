...
MAX_CLEARANCE_OBJECT_BBOX_AREA_M2 = 100.0
MAX_CLEARANCE_OBJECT_BBOX_SIDE_M = 12.0
...
def load_episode_static_reference_navigable(scene_path: Path) -> Tuple[np.ndarray, str]:
    collision_path = scene_path / "collision_navigable.npy"
    if collision_path.is_file():
        return np.load(collision_path).astype(bool), "collision_navigable.npy"
    return np.load(scene_path / "navigable.npy").astype(bool), "navigable.npy"
...
def filter_start_clearance_objects(
    ...
    for obj in objects:
        ...
        if bbox_min is None or bbox_max is None:
            continue
        extent_x = max(0.0, float(bbox_max[0]) - float(bbox_min[0]))
        extent_y = max(0.0, float(bbox_max[1]) - float(bbox_min[1]))
        if (
            extent_x > MAX_CLEARANCE_OBJECT_BBOX_SIDE_M
            or extent_y > MAX_CLEARANCE_OBJECT_BBOX_SIDE_M
            or extent_x * extent_y > MAX_CLEARANCE_OBJECT_BBOX_AREA_M2
        ):
            continue
...
def build_static_execution_navigable(
    navigable: np.ndarray,
    map_info: MapInfo,
    clearance_objects: Sequence[Mapping],
    *,
    ...
    reference_radius = float(reference_radius_m)
    runtime_margin = float(runtime_planning_clearance_m)
    if robot_radius <= 0.0 or reference_radius < 0.0 or runtime_margin < 0.0:
        raise ValueError("static execution-clearance radii are invalid")
    object_cleared = build_clearance_mask(
        navigable,
        map_info,
        clearance_objects,
        runtime_margin,
    )
    # The collision reference has already been inflated by reference_radius.
    # Keep the online planner's soft runtime margin out of the physical
    # collision contract: on a 5 cm grid a 1 cm margin raises the EDT cutoff
    # from 10 cm to the next representable 15 cm band.
    extra_clearance_m = max(0.0, robot_radius - reference_radius)
    if extra_clearance_m <= 0.0:
        execution_navigable = np.asarray(object_cleared, dtype=bool).copy()
    else:
        clearance_m = (
            ndimage.distance_transform_edt(np.asarray(object_cleared, dtype=bool))
            * float(map_info.resolution_m)
        )
        execution_navigable = np.asarray(object_cleared, dtype=bool) & (
            clearance_m >= extra_clearance_m - 1.0e-9
        )
    metadata = {
        "robot_radius_m": robot_radius,
        "static_reference_radius_m": reference_radius,
        "runtime_planning_clearance_m": runtime_margin,
        "static_execution_extra_clearance_m": extra_clearance_m,
        "static_reference_navigable_cells": int(np.count_nonzero(navigable)),
        "static_object_cleared_navigable_cells": int(
            np.count_nonzero(object_cleared)
        ),
        "static_execution_navigable_cells": int(
            np.count_nonzero(execution_navigable)
        ),
        "static_execution_contract": (
            "usd_visible_face_collision_then_reference_radius_delta"
        ),
    }
    if not np.any(execution_navigable):
        raise ValueError("static execution-clearance contract removed every cell")
    return execution_navigable, metadata
...
def select_longest_instance_astar_episode(
    ...
    static_reference_radius_m: float = 0.05,
    top_k: int = 10,
    ...
    with open(scene_path / "scene_metadata.json", "r", encoding="utf-8") as handle:
        metadata = json.load(handle)
    map_info = load_map_info(str(scene_path / "map_info.json"))
    selection_reference_navigable = np.load(scene_path / "navigable.npy").astype(bool)
    navigable, _selection_contract = build_static_execution_navigable(
        selection_reference_navigable,
        map_info,
        objects_for_start_clearance,
        robot_radius_m=float(robot_radius_m),
        reference_radius_m=float(static_reference_radius_m),
        runtime_planning_clearance_m=float(min_planning_clearance_m),
    )
    collision_reference_navigable, static_reference_navigation_source = (
        load_episode_static_reference_navigable(scene_path)
    )
    _, execution_contract = build_static_execution_navigable(
        collision_reference_navigable,
        map_info,
        [],
        robot_radius_m=float(robot_radius_m),
        reference_radius_m=float(static_reference_radius_m),
    ...
    episode = {
        ...
        "metadata": {
            ...
            "static_reference_navigation_source": static_reference_navigation_source,
            **execution_contract,
            "start_clearance_object_count": int(len(objects_for_start_clearance)),
            "start_clearance_excluded_categories": sorted(START_CLEARANCE_EXCLUDE_CATEGORIES),
            "top_candidates": [
