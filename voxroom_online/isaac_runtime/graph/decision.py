...
class SGNavDecision:
    def __init__(
        ...
        if self.frontier_selection_mode not in {
            "sgnav",
            "nearest",
            "random",
            "tvars_original",
    ...
    def choose_frontier(self, frontier_clusters: List[FrontierCluster]) -> DecisionResult:
        if self.frontier_selection_mode == "tvars_original":
            raise RuntimeError(
                "TVARS original frontier targets must come from the original "
                "topology policy"
            )
    ...
    def choose_navigation_target(
        ...
        ignore_goal_candidates: bool = False,
    ) -> NavigationDecision:
        if self.frontier_selection_mode == "tvars_original":
            raise RuntimeError(
                "TVARS original navigation bypasses SGNavDecision target selection"
            )
        ...
        candidate = None if ignore_goal_candidates else self.select_goal_candidate(
            object_memory,
            goal_category,
            current_pose,
    ...
    def _frontier_target_cells(
        ...
        *,
        ...
        if dist_lookup is not None and dist_lookup.shape != planner.traversible.shape:
            dist_lookup = None

        def is_reachable(cell: Tuple[int, int]) -> bool:
            target = (int(cell[0]), int(cell[1]))
            if dist_lookup is not None:
                if not planner.inside(target):
                    return False
                return bool(np.isfinite(float(dist_lookup[target])))
            return bool(planner.plan(current_grid, [target]).path)

        center_reachable = is_reachable(center)
        meta: Dict[str, object] = {
            ...
            "frontier_target_reachability_source": "distance_map" if dist_lookup is not None else "planner_plan",
        }
        if center_reachable:
